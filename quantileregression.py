...
class Worker():
    def __init__(self,env,name,s_size,a_size,trainer,model_path,global_episodes,replaymemory):
    ...
    def train(self,rollout,sess,gamma,ISWeights):
        ...
        dones             = rollout[:,4]
        #print 'ssssssssssssssssssssssssssssssssssssssssssssssssssssssss',rewards
        ...
        for i in range(len(rewards)):
            q_target_batch.append(q_target[i] * gamma + rewards[i])
        ...
        feed_dict = {self.local_AC.inputs:np.vstack(observations),
                     ...
                     self.local_AC.ISWeights:isweight}
        u,l,g_n,v_n,_ = sess.run([self.local_AC.u,
                                  self.local_AC.loss,
...
for i in range(num_workers):
    env = get_env(task)
    workers.append(Worker(env,i,s_size,a_size,trainer,model_path,global_episodes,replaymemory))
